...
import com.sh.im.http.dao.ImChatGroupUserMapper;
import com.sh.im.http.dao.ImChatUserFriendMapper;
import com.sh.im.http.entity.ImChatGroupUser;
import com.sh.im.http.entity.ImChatUserFriend;
import com.sh.im.websocket.ImConfigConst;
...
import com.sh.utils.mail.MailUtil;
import com.sh.vo.BaseRequestVO;
import com.sh.vo.UserVO;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public PoetryResult<UserVO> updateUserInfo(UserVO user) {
        if (StringUtils.hasText(user.getUsername())) {
            ...
            Integer count = lambdaQuery().eq(User::getUsername, user.getUsername()).ne(User::getId, PoetryUtil.getUserId()).count().intValue();
            if (count != 0) {
                return PoetryResult.fail("用户名重复！");
            }
        }
    ...
    public PoetryResult<UserVO> updateSecretInfo(String place, Integer flag, String code, String password) {
        ...
        if (flag == 1) {
            Integer count = lambdaQuery().eq(User::getPhoneNumber, place).count().intValue();
            if (count != 0) {
                return PoetryResult.fail("手机号重复！");
            }
            ...
            Integer count = lambdaQuery().eq(User::getEmail, place).count().intValue();
            if (count != 0) {
                return PoetryResult.fail("邮箱重复！");
            }
            Integer codeCache = (Integer) PoetryCache.get(CommonConst.USER_CODE + PoetryUtil.getUserId() + "_" + place + "_" + flag);
