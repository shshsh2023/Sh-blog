...
<script setup>
import {defineAsyncComponent, getCurrentInstance, nextTick, onMounted, onUnmounted, ref, watch} from "vue";
import $ from "jquery"
...
watch(scrollTop, (scrollTop, oldScrollTop) => {
...
const subscribeLabel = () => {
  ...
  this.$confirm('确认' + (subscribe.value ? '取消订阅' : '订阅') + '专栏【' + article.value.label.labelName + '】？' + (subscribe.value ? "" : "订阅专栏后，该专栏发布新文章将通过邮件通知订阅用户。"), subscribe.value ? "取消订阅" : "文章订阅", {
...
const deleteTreeHole = (id) => {
  ...
  this.$confirm('确认删除？', '提示', {
    ...
    http.get(constant.baseURL + "/weiYan/deleteWeiYan", {id: id.value})
        .then(() => {
...
const getArticle = (password) => {
  http.get(constant.baseURL + "/article/getArticleById", {id: id.value, password: password})
      .then((res) => {
        if (!common.isEmpty(res.data)) {
          ...
          if (!common.isEmpty(password)) {
            localStorage.setItem("article_password_" + id.value, password);
          }
          showPasswordDialog.value = false;
          if (!common.isEmpty(store.state.currentUser) && !common.isEmpty(store.state.currentUser.subscribe)) {
            subscribe.value = JSON.parse(store.state.currentUser.subscribe).includes(article.value.labelId);
          }
        }
      })
      .catch((error) => {
        if ("密码错误" === error.message.substr(0, 4)) {
          ...
          tips.value = error.message.substr(4);
