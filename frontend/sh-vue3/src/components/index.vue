          ...
          <div class="page-container">
            ...
            <div class="recent-posts">
              <div class="announcement background-opacity">
                ...
                <div>
                  <div v-for="(notice, index) in common.pushNotification(store.state.webInfo.notices, true)"
                       :key="index">
                    {{ notice }}
                  </div>
                </div>
...
let printerInfo = ref("你看对面的青山多漂亮");
...
onMounted(() => {
  ...
  getSortArticles();
  setTimeout(() => {
    push.value = common.pushNotification(store.state.webInfo.notices, false);
    // console.log(push.value)
    // pushDialogVisible.value = true;
    if (!common.isEmpty(push.value)) {
      if ("0" !== localStorage.getItem("showPushNotification_" + push.value['链接'])) {
        pushDialogVisible.value = true;
        localStorage.setItem("showPushNotification_" + push.value['链接'], "0");
      }
    }
  }, 2000);
})
...
const sortInfo = ref(store.state.sortInfo)
...
const getArticles = async () => {
  await http.post(constant.baseURL + "/article/listArticle", pagination.value)
      .then((res) => {
